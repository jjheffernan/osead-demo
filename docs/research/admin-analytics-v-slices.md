...
**Phase 3 (Wave G — AFK continuation):** densify Overview into a broker one-stop shop (closed deals + leading market + top-3 staff pulse); client JS budget doc; CollectionPage JSON-LD; janitor. Still zero chart deps / no Card swap / no uPlot unless a later ticket asks. Conversion bar: a broker should want this view enough to white-label the template. Cloudflare deploy stays in TODO Later until Now is clear.
